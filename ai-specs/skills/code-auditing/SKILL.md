...
- Dynamic imports (`importlib.import_module(variable)`, `__import__`)
- Framework / registration patterns (decorators, argparse `command_class` factory defaults)
- Re-exports for public API (`__init__.py`, `__all__`)
- Entry points (`[project.scripts]`, `python -m <job_package>` / `__main__.py`)
