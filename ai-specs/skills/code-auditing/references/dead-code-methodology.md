...
Dead code is code that exists in the codebase but is never executed. It increases maintenance burden and cognitive load, and it hides real intent. This methodology uses specialized tools combined with agent verification to filter false positives.
...
#### 2. Framework / registration magic
```python
# Registered via a decorator (looks unused, invoked by the framework)
@app.command()
def run(): ...

# Subcommand handler wired by argparse via set_defaults(command_class=...)
class KrakenCommand(BaseCommand): ...  # referenced only as a factory default

# Referenced by name / entry point (setup.cfg/pyproject [project.scripts], plugin registry)
def main(): ...  # `<job> = <job_package>.job:main`
```
...
3. **Check framework / registration patterns**:
   - Is it registered via a decorator (CLI command, event/handler registry)?
   - Is it a Command subclass wired only as an argparse `command_class` factory default?
   - Is it referenced by name — an entry point (`[project.scripts]`), a dynamic import, or a
     string key in a dispatch table?
