...
> **Note (this project, sdd-job):** the text below describes specboot's original
> workflow, written against OpenSpec ~1.3.1 (`/new`, `/ff`, `/apply`, `/verify`, `/archive`).
> This repo has **OpenSpec 1.5.0** installed, which renamed/merged those CLI commands and
> dropped the standalone `/verify` step. The actual commands to use here are:
>
> 1. `/enrich-us` (optional) — refine a vague user story or idea (specboot skill, unchanged)
> 2. `/opsx:propose` — replaces `/new` + `/ff`: creates the OpenSpec change with all artifacts
>    (proposal.md, design.md, tasks.md) in one step
> 3. `/opsx:apply` — replaces `/apply`: implement tasks one by one
> 4. `/adversarial-review` — replaces **both** `/verify` and `/adversarial-review`: runs
>    mechanical verification (tests/lint/build/task-completion, Step 0) followed by the
>    adversarial pass (specboot skill, extended — see `ai-specs/skills/adversarial-review/SKILL.md`)
> 5. `/opsx:archive` — replaces `/archive`: archive the completed change
> 6. `/commit` — unchanged: create focused commit(s) and manage the Pull Request
>
> The original (superseded) description follows for historical reference.
...
```bash
# Original specboot example (OpenSpec ~1.3.1) — kept for reference, do not run as-is:
/enrich-us SCRUM-10
/ff SCRUM-10
/apply SCRUM-10
/verify SCRUM-10
...
# Actual sequence for this project (OpenSpec 1.5.0):
/enrich-us SCRUM-10
/opsx:propose SCRUM-10
/opsx:apply SCRUM-10
/adversarial-review SCRUM-10   # runs mechanical verification + adversarial pass
/opsx:archive SCRUM-10
/commit
```
